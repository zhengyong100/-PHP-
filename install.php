...
<head>
    ...
    <p align="center">
        ...
        echo "<br />加入默认分类.....";
        $sql = "select * from ".$qianzui."account_class where classname='默认收入'";
        $query = mysqli_query($conn,$sql);
        ...
        $field = mysqli_query($conn,'Describe '.$qianzui.'user cpassword');
        //像数据库发送SQL语句
        ...
        if ($attitle) {
            ...
            $sql = "insert into ".$qianzui."user (username, password,email,utime) values ('admin', '098f6bcd4621d373cade4e832627b4f6','i@xptt.com','$utime')";
            $query = mysqli_query($conn,$sql);
