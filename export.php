...
if ($action == 'import') {
  ...
  if($chongfu[actime]==$timechongfu){
    ...
    for ($i = 1; $i < $len_result; $i++) {
        //循环获取各字段值
        $classify = iconv('gbk', 'utf-8', $result[$i][0]);
        //中文转码
        $shouzhi = iconv('gbk', 'utf-8', $result[$i][1]);
        if ($shouzhi == "收入") {
            $shouzhi = "1";
        } else {
            $shouzhi = "2";
    ...
    while ($row = mysqli_fetch_array($result)) {
        ...
        $classify = iconv('utf-8','gbk',$classinfo['classname']);
        if ($classinfo[classtype] == 1) {
            $shouzhi = iconv('utf-8','gbk',"收入");
        } else {
            $shouzhi = iconv('utf-8','gbk',"支出");
        }
        $jine = $row['acmoney'];
        $shijian = date("Y-m-d H:i",$row[actime]);
        $beizhu = iconv('utf-8','gbk',$row[acremark]);
